  ...
  methods: {
    login() {
      ...
      axios.post('/users/login', {
        ...
        if (res.status == "1") {
          ...
          this.$store.commit('updataNickName', res.result.userName)
          this.getCartCount()
        } else {
          this.errorTip = true
        }
    ...
    logout() {
      axios.post('/users/logout').then((response) => {
        ...
        if (res.status == "1") {
          // this.nickName = ''
          this.$store.commit('updataNickName', '')
    ...
    checkLogin() {
      axios.get('/users/checkLogin').then((response) => {
        ...
        if (res.status == '1') {
          ...
          this.$store.commit('updataNickName', res.result)
        }
      })
    },
    getCartCount() {
